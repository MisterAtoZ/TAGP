...
start_3pipes_water_pump_flowmeter_heatex() ->
	...
	{ok, FlowMeterInst} = flowMeterInst:create(self(buildSystemFlowMeterTyp,Pipe2InstPID, FlowMeterCMD),
	
	%Create heat exchanger
	%?debugFmt("Creating heat exchanger~n",[]),
	{ok, HeatExTyp} = heatExchangerTyp:create(),
	%?debugFmt("Heat Exchanger Type ~p~n",[HeatExTypbuildSystem,
	Difference = 0.9,
	HE_link_spec = #{delta => Difference},
	{ok, HeatEx} = heatExchangerInst:create(self(), buildSystematExTyp, Pipe3InstPID, HE_link_spec),
	%?debugFmt("Heat Exchanger Inst ~p~n",[HeatEx]),buildSystem
